...
class SemanticFallbackTests(unittest.TestCase):
    ...
    def test_farm_wide_mean_temperature_stays_deterministic(self) -> None:
        ...
        self.assertEqual((fast.intent, fast.measurement), ("farm-average", "air_temperature_c"))
        self.assertFalse(needs_semantic_interpretation(question, fast))

    def test_general_animal_health_question_fails_open_to_learning(self) -> None:
        ...
        self.assertIn("Milk fever", response.answer)
    ...
    def test_ambiguous_rename_fails_closed_without_a_write(self, prepare_rename) -> None:
        response = asyncio.run(ask(AskRequest(question="Rename Paddock A")))
        self.assertEqual(response.intent, "semantic-clarification")
        self.assertIn("not confident", response.answer.casefold())
        prepare_rename.assert_not_called()
        self.assertIsNone(response.confirmation_id)

    def test_research_request_is_labelled_without_claiming_live_research(self) -> None:
        response = asyncio.run(ask(AskRequest(question="Could you research current sources about milk fever in cows?")))
        self.assertEqual(response.intent, "agriculture-research")
        self.assertTrue(response.answer.startswith("No live web research was performed."))
        self.assertEqual(response.source_category, "combined")

    def test_general_question_does_not_require_the_farm_database(self) -> None:
        response = asyncio.run(ask(AskRequest(question="What is mastitis?")))
        self.assertEqual(response.intent, "agriculture-learning")
        self.assertEqual(response.source_tier, "first-class-trusted")

    def test_non_action_classifier_uncertainty_still_gets_a_learning_response(self) -> None:
