...
Supported operations include current values, current farm-wide averages across the latest active-paddock snapshot, current highest/lowest paddock rankings where the catalogue permits the operation, historical minimum/maximum/average, rainfall total, change, range, simple first-to-last rate/trend, simple two-standard-deviation anomaly flagging, paddock comparisons, and compact summaries. In learner language, **field/fields** is accepted as a conversational alias for **paddock/paddocks**; it does not create a separate database entity. Plain `temperature` means air temperature, while `soil temperature` remains distinct. Every historical request has an explicit period: last hour, last 24 hours, last seven days, or local `today`/`this morning` (Pacific/Auckland calendar boundaries converted to UTC before query).

A current farm-wide average such as `What is the average temperature across all fields?` uses exactly one latest complete reading per active paddock and calculates the arithmetic mean deterministically in Python. Likewise, `What is the highest temperature?`, `Which field is coldest?`, `Which paddock is wettest?`, and equivalent reviewed highest/lowest forms select the result from that same verified current snapshot. These are current cross-paddock operations, not historical trends and not calculations performed by the language model.
