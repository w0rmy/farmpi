...
| Interpretation | A reviewed `QuestionRoute` selects only approved operations. It recognises farm-wide lists/counts before paddock extraction, normalises common measurement language, and carries only minimal explicit follow-up context. | `app/question_router.py` |
...
| Educational grounding | Version-controlled concept cards provide Simple, Normal, and Technical explanations and limitations independently of telemetry. | `app/education.py`, `app/learning.py`, `app/guidance.py` |
...
| Local model boundary | Qwen may phrase supplied verified facts. It does not choose a route, query MariaDB, calculate, rename a paddock, or make causal/agronomic claims. Direct deterministic answers do not need a model call. | `app/app.py`, `docs/grounding-and-guardrails.md` |
